...
import { Download, RadioTower, Search, Ship } from "lucide-react"
...
const STATUS_STYLES: Record<TrackingStatus, string> = {
  arrived: "border-emerald-500/25 bg-emerald-500/10 text-emerald-400",
  "on-water": "border-sky-500/25 bg-sky-500/10 text-sky-400",
  origin: "border-foreground/10 bg-foreground/[0.04] text-muted-foreground",
  "not-found": "border-rose-500/25 bg-rose-500/10 text-rose-400",
}
    ...
    <div className="flex flex-col gap-4">
      ...
      <Card className="gap-4 rounded-2xl border-foreground/[0.06] bg-foreground/[0.02] p-6 shadow-none backdrop-blur-sm">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          {result && (
            <Button
              onClick={() => downloadExcel(result.rows)}
              size="sm"
              variant="outline"
              className="gap-1.5 rounded-full border-foreground/15 bg-transparent px-5 text-xs hover:bg-foreground/[0.05]"
            >
          ...
          {result && !error && (
            <span className="text-xs tabular-nums text-muted-foreground">
              ...
              {result.notFound.length > 0 && (
                <span className="text-rose-400/80"> · {result.notFound.length} not found</span>
              )}
            </span>
          )}
        </div>
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-center gap-2">
              ...
              <span className="ml-1 text-xs tabular-nums text-muted-foreground/50">
                {rows.length} of {result.rows.length}
              </span>
            </div>
