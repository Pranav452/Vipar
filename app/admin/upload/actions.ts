...
export async function analyzeUpload(_prev: AnalyzeState, formData: FormData): Promise<AnalyzeState> {
  ...
  if (!(file instanceof File) || file.size === 0) {
    return { error: "Choose the operations sheet file first." }
  }
  ...
  try {
    ...
    if (report.workOrders === 0) {
      return { error: "No work orders found in the file — check that it is the operations sheet." }
    }
...
export async function commitUpload(_prev: CommitState, formData: FormData): Promise<CommitState> {
  ...
  try {
    ...
    await audit("dataset-upload", {
      user: session.u,
      file: source,
      workOrders: String(shipments.length),
      asOf,
    ...
    return { success: true, versionId, workOrders: shipments.length, warnings }
  } catch (err) {
