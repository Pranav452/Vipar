...
export async function addAllowedIp(formData: FormData): Promise<void> {
  ...
  // exact IPv4/IPv6-ish or prefix ending with a dot
  if (!/^[0-9a-fA-F.:]{2,45}$/.test(ip)) return
  await withRetry(() => sql`
    INSERT INTO allowed_ips (ip, label) VALUES (${ip}, ${label})
...
export async function deleteAllowedIp(formData: FormData): Promise<void> {
  ...
  const rows = (await withRetry(() => sql`DELETE FROM allowed_ips WHERE id = ${id} RETURNING ip`)) as { ip: string }[]
