...
export async function login(_prev: LoginState, formData: FormData): Promise<LoginState> {
  ...
  if (verified.role === "client") {
    if (!(await ipAllowedForClient(ip))) {
      ...
      return { error: "This network is not authorised for the VIPAR portal. Contact LINKS." }
    }
    ...
    if (!device.ok) {
      ...
      return {
        error: "This device is not authorised for the VIPAR portal. Contact LINKS to register it.",
      }
    }
    deviceId = device.deviceId
    await audit(device.isNew ? "login-ok-new-device" : "login-ok", { user, deviceId })
  } else {
    if (!deviceId) deviceId = "admin"
    await audit("login-ok-admin", { user })
  }
  ...
  const target = verified.role !== "admin" && from.startsWith("/admin") ? roleHome : from
  redirect(target)
}
